...
export const getStartSites = (per_page: number, page: number, ownerName?: string, query?: string) => {
...
export const getEndSites = (per_page: number, page: number, ownerName?: string, query?: string) => {
    const params = new URLSearchParams();
    if (ownerName) {
        params.append('ownerName', ownerName);
    }
...
export const getCategories = (per_page: number, page: number, query?: string) => {
    const params = new URLSearchParams();
    if (query) {
        params.append('query', query);
    }
    return apiClient.get(`/parameter/goods_list/${per_page}/${page}?${params.toString()}`);
};
...
export const getFleets = (per_page: number, page: number, query?: string) => {
    ...
    return apiClient.get(`/parameter/vehicle_list/${per_page}/${page}?${params.toString()}`);
};
...
export const getProjectOwners = (perPage: number, page: number, query?: string) => {
    const params = new URLSearchParams();
    if (query) {
        params.append('query', query);
    }
    return apiClient.get(`/parameter/project_list/${perPage}/${page}?${params.toString()}`);
};
...
export const getPaymentMethods = (per_page: number, page: number, query?: string) => {
    const params = new URLSearchParams();
    if (query) {
        params.append('query', query);
    }
    return apiClient.get(`/parameter/pay_list/${per_page}/${page}?${params.toString()}`);
};
...
export const getLoads = (per_page: number, page: number, query?: string) => {
    ...
    return apiClient.get(`/parameter/load_list/${per_page}/${page}?${params.toString()}`);
};
