<template>
    <div class="advance-entry">
      ...
      <el-card>
        ...
        <el-table :data="advances" style="width: 100%">
          <el-table-column prop="vehicle.driver" label="司机">
  ...
  import { addAdvance, delAdvance, getAdvances, updateAdvance } from '@/services/finance';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const advances = ref<{ id: number, vehicle: { id: number, driver: string }, amount: number, advance_time: string }[]>([]);
      const editingAdvance = ref({ vehicle_id: 0, amount: 0, advance_time: '' });
      ...
      const editParameter = (advance: { id: number, vehicle: { id: number, driver: string }, amount: number, advance_time: string }) => {
        editingId.value = advance.id;
        editingAdvance.value = { vehicle_id: advance.vehicle.id, amount: advance.amount, advance_time: advance.advance_time };
      };
      ...
      const cancelEdit = () => {
        ...
        editingAdvance.value = { vehicle_id: 0, amount: 0, advance_time: '' };
      };
